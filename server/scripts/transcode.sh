...
# -map 0:v:0        select first video stream only (skips spatial/metadata tracks)
# -map 0:a:0        select first audio stream only (skips unknown apac spatial audio)
# -pix_fmt yuv420p  convert 10-bit HEVC to 8-bit for broad compatibility
# -vf transpose     auto-rotate based on display matrix metadata
